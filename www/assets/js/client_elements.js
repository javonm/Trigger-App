...
CheckboxElement.prototype.setLabelFont = function(font) {
	if(font === undefined || font === null) {
		return;
	}
	
	this.zebraUIElement.find("//zebra.ui.MLabel").setFont(font.toString());
};

CheckboxElement.prototype.setLabelColor = function(color) {
	if(color === undefined || color === null) {
		return;
	}
	
	this.zebraUIElement.find("//zebra.ui.MLabel").setColor(color);
};
...
CheckboxElement.prototype.updateState = function(state) {
	...
	if(state.labelFont !== undefined) {
		this.setLabelFont(state.labelFont);
	}
	
	if(state.labelColor !== undefined) {
		this.setLabelColor(state.labelColor);
	}
...
//Target
ComboboxElement.prototype.setIndex = function(index) {
	if(index >= this.zebraUIElement.list.model.d.length) {
		return;
	}
	
	var value = this.zebraUIElement.list.model.d[index];
	this.zebraUIElement.setValue(value);
};
...
//Source
ComboboxElement.prototype.getIndex = function() {
	return this.zebraUIElement.list.model.d.indexOf(this.zebraUIElement.getValue());
};

//Source
ComboboxElement.prototype.getValues = function() {
	return this.zebraUIElement.list.model.d.slice();
};
...
//Helper
LocalStorageElement.prototype.loadValue = function() {
	try {
...
SliderElement.prototype.updateState = function(state) {
	...
	if(state.value !== undefined && this.valueLocked === false) {
		this.zebraUIElement.setValueNoEvent(state.value);
	}
...
TaskElement.prototype.run = function(runIn, runEvery) {
	...
	this.zebraTask.run(this.currentRunIn, this.currentRunEvery);
};
