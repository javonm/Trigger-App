function ConnectionElement(parent) {
	...
	//Trigger
	this.rssiRecieved = function() {
	};
	
	//Trigger
	this.errorRSSIRecieved = function() {
	};
	...
	//Target
	this.retrieveRSSI = function(address) {
		var currentConnectionElement = this;
		
		if(address === undefined) {
			address = this.parent.currentlyConnectedAddress;
		}
		
		bluetoothle.rssi(
			// success
			function(retObject) {
				currentConnectionElement.currentRSSI = retObject.rssi;
				currentConnectionElement.rssiRecieved();
				
			}, 
			// err
			function(retObject) {
				currentConnectionElement.errorRSSIRecieved();
			}, 
			{"address":address}
		);
	};	
...
function parseIEEE11073Float32(data) {
	if(data.length < 4) {
		return null;
	}
	...
	function _unsignedToSigned(unsigned, size) 
	{
	...
	var mantissa = _unsignedToSigned(_ToInt(data[0]) + (_ToInt(data[1]) << 8) + (_ToInt(data[2]) << 16), 24);
	var exponent = _unsignedToSigned(_ToInt(data[3]), 8);

	return (mantissa * Math.pow(10, exponent));
}

function GATTHealthThermometerElement(parent) {
	...
	this.currentIntermediateValue = null;

	parent.connections.push(this);
}

//Trigger
GATTHealthThermometerElement.prototype.connecting = function() {
};

//Trigger
GATTHealthThermometerElement.prototype.connected = function() {
};

//Trigger
GATTHealthThermometerElement.prototype.disconnecting = function() {
};

//Trigger
GATTHealthThermometerElement.prototype.disconnected = function() {
};

//Trigger
GATTHealthThermometerElement.prototype.rssiRecieved = function() {
};

//Trigger
GATTHealthThermometerElement.prototype.errorRSSIRecieved = function() {
};

GATTHealthThermometerElement.prototype.bleCallbackHandler = function(e) {
	
	var currentElement = this;
	this.currentValue = e;
	
	if(this.currentValue.status === "discovered") {
		
		var temperatureMeasurementIndicationOptions = {"service":"1809", "characteristic":"2A1C", "address":this.parent.currentlyConnectedAddress};
		
		bluetoothle.subscribe(
			function(retObject) {
				currentElement.temperatureMeasurementIndicationCallback(retObject);
			}, 
			
			function(retObject) {
				
			}, temperatureMeasurementIndicationOptions);
		
		var intermediateTemperatureNotificationOptions = {"service":"1809", "characteristic":"2A1E", "address":this.parent.currentlyConnectedAddress};
		
		bluetoothle.subscribe(
			function(retObject) {
				currentElement.intermediateTemperatureNotificationCallback(retObject);
			}, 
			
			function(retObject) {
				
			}, intermediateTemperatureNotificationOptions);
	}
};

//Helper Method
GATTHealthThermometerElement.prototype.temperatureMeasurementIndicationCallback = function(retObject) {
	if(retObject.status === "subscribedResult") {
		...
		var temperatureValue = parseIEEE11073Float32([data.charCodeAt(1), data.charCodeAt(2), data.charCodeAt(3), data.charCodeAt(4)]);
		
		this.currentMeasurementValue = [unitFlag, temperatureValue];
		this.onTemperatureMeasurement();
	}
};

//Helper Method
GATTHealthThermometerElement.prototype.intermediateTemperatureNotificationCallback = function(retObject) {
	if(retObject.status === "subscribedResult") {
		var data = atob(retObject.value);
		...
		this.currentIntermediateValue = [unitFlag, temperatureValue];
		this.onIntermediateTemperature();
	}
};

//Trigger
GATTHealthThermometerElement.prototype.onIntermediateTemperature = function(e) {
	
};

//Trigger
GATTHealthThermometerElement.prototype.onTemperatureMeasurement = function(e) {
	
};

//Source
GATTHealthThermometerElement.prototype.getMeasuredValue = function() {
	return this.currentMeasurementValue;
};

//Source
GATTHealthThermometerElement.prototype.getIntermediateValue = function() {
	return this.currentIntermediateValue;
};

//Target
GATTHealthThermometerElement.prototype.trigger = function() {
