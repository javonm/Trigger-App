...
PhoneGapBLEClient.prototype.initialize = function() {
	...
	if (env.deviceFail) {
		if (env.debug) console.log ("Device state failure, loading fallback");
...
PhoneGapBLEClient.prototype.getProjectList = function() {
	...
	var url = env.apiUrl + "/auth/fetch_projects.php?data=" + session;	
...
PhoneGapBLEClient.prototype.registerDeviceToCloud = function() {
	...
	//TODO: What if creds comes back null or undefined?
...
PhoneGapBLEClient.prototype.completeConnection = function() {
	...
	if(this.cloudEnabled && this.cloudAvailable) {
		this.getCloudCredentials(function(uuid, token) {
		    ...
		    //TODO: what if undefined - possible in standalone app..  
		    //TODO: add UX debug logging for failure reasons
			...
			//TODO: ok so we get a message if we register, but what about if we fail?
		...
		if(this.cloudEnabled !== true) {
			console.log("cloudEnabled: false");
		}
		
		if(this.cloudAvailable !== true) {
			console.log("cloudAvailable: false");
		}
...
// 					paramsObj = {"address":this.currentlyConnectedAddress, "clearCache": true};
			...
			case "services":
				...
				for (var j = 0; j < serviceUuids.length; j++) {
					...
					if(serviceUuids[j].toLowerCase() === "bfe433cf-6b5e-4368-abab-b0a59666a402") {
